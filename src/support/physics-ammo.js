...
init: function () {
	...
	this.offset = this.data.offset || new THREE.Vector3();
	this.posSync = new THREE.Vector3();
	this.rotSync = new THREE.Quaternion();
},
update: function () {
...
tick: function (time, timeDelta) {
	if(this.data.sync){
		if(this.el.body){
			...
			this.auxl[this.data.auxlObj].GetEl().object3D.getWorldQuaternion(this.rotSync)
			//Offset
			this.posSync.add(this.offset);
			//Physics Body Update
			this.el.object3D.position.copy(this.posSync);
			this.el.object3D.quaternion.copy(this.rotSync);
		}
	}
},
});
